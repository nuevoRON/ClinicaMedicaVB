...
Public Class BackupForm
    ...
    Public Sub New()
        ...
        lblEstado.Text="Seleccione una copia de la lista para restaurarla."
    ...
    Private Function CarpetaCopias() As String
    ...
    Private Sub CrearCopia(sender As Object,e As EventArgs)
        Try
            ...
            Using dlg As New SaveFileDialog()
                ...
                dlg.InitialDirectory=CarpetaCopias()
                dlg.FileName=$"clinica_backup_{DateTime.Now:yyyyMMdd_HHmmss}.db"

                If dlg.ShowDialog()<>DialogResult.OK Then Return

                Dim destino=dlg.FileName
                If Path.GetFullPath(destino).Equals(Path.GetFullPath(Database.DbPath),StringComparison.OrdinalIgnoreCase) Then
                    MessageBox.Show("No puede sobrescribir la base de datos activa.","Copia")
                    Return
                End If

                File.Copy(Database.DbPath,destino,True)
    ...
    Private Sub RestaurarCopia(sender As Object,e As EventArgs)
        ...
        If MessageBox.Show("La restauración reemplazará la base de datos actual. Se recomienda crear una copia antes de continuar." & Environment.NewLine & Environment.NewLine & "¿Desea continuar?","Confirmar restauración",MessageBoxButtons.YesNo,MessageBoxIcon.Warning)<>DialogResult.Yes Then Return

        Try
            ' Copia preventiva antes de restaurar.
            Dim preventiva=Path.Combine(CarpetaCopias(),$"antes_restaurar_{DateTime.Now:yyyyMMdd_HHmmss}.db")
            File.Copy(Database.DbPath,preventiva,True)

            ' Cerrar conexiones antes de sustituir el archivo.
            GC.Collect()
            ...
            MessageBox.Show("Restauración completada. Para asegurar que todos los módulos trabajen con la información restaurada, cierre y vuelva a abrir el sistema.","Restauración completada",MessageBoxButtons.OK,MessageBoxIcon.Information)
            CargarCopias()
        Catch ex As Exception
    ...
    Private Sub CargarCopias()
        ...
        For Each archivo In Directory.GetFiles(CarpetaCopias(),"*.db")
